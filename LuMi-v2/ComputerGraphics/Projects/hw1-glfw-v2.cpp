// hw1-glfw.cpp - OpenGL 3.3 Core + GLFW version
// Fixes for "UI text not showing":
//  1) Disable face culling (stb_easy_font triangle winding can be "backwards" under Y-down UI proj)
//  2) Ensure UI blending + opaque text color
//  3) Ensure UI projection uses framebuffer size (HiDPI-safe)
...
// UI layout in pixel space (origin top-left, y down)
...
static GLuint dynLineVAO=0, dynLineVBO=0;
static GLuint dynTriVAO=0, dynTriVBO=0;

// ---------------- Common draw ----------------
static void useProgram() { glUseProgram(gProgram); }
...
static void addTextTriangles(std::vector<float>& outXY, float x, float y, const char* text) {
    ...
    for (int q = 0; q < quads; q++) {
        ...
        // Keep this winding; we disable culling so it will render regardless.
...
static void drawTreeRowPX(
...

    {
        ...
        // IMPORTANT: Opaque alpha for text
        setColor(isCategory ? 1.0f : 0.9f, isCategory ? 1.0f : 0.9f, isCategory ? 0.0f : 0.9f, 1.0f);
...
static void drawUI() {
    // Key fix: disable culling so stb_easy_font triangles are not discarded
...
static void display() {
    ...
    // Scene transform (pivot at (1.0, -0.5))
    ...
    drawQuad(mul(scene, rectModelNDC(-1.0f, -0.8f, 2.0f, 0.3f)));

    // Truck
    Mat4 truck = mul(scene, Mat4::translate(truckX, -0.5f + wheelRadius, 0.0f));

    setColor(0.3f,0.3f,0.8f,1.0f);
    drawQuad(mul(truck, rectModelNDC(-0.5f, 0.0f, 1.0f, 0.15f)));

    setColor(0.4f,0.4f,0.9f,1.0f);
    drawQuad(mul(truck, rectModelNDC(-0.5f, 0.15f, 0.4f, 0.2f)));

    setColor(0.5f,0.5f,1.0f,1.0f);
    drawQuad(mul(truck, rectModelNDC(-0.1f, 0.15f, 0.4f, 0.25f)));

    setColor(0.7f,0.9f,1.0f,1.0f);
    drawQuad(mul(truck, rectModelNDC(-0.05f, 0.25f, 0.3f, 0.15f)));

    // Wheels solid when not wireframe
    if (!wireframe) glPolygonMode(GL_FRONT_AND_BACK, GL_FILL);

    // Circle drawing uses the circle mesh scaled; we pass model as MVP directly (NDC)
    auto drawWheel = [&](float tx){
        Mat4 w = mul(truck, Mat4::translate(tx, 0.0f, 0.0f));
        w = mul(w, Mat4::rotateZ(wheelAngle));
        // outer
        setColor(0.1f,0.1f,0.1f,1.0f);
        drawCircle(mul(w, Mat4::scale(wheelRadius, wheelRadius, 1.0f)));
        // inner
        setColor(0.8f,0.8f,0.8f,1.0f);
        drawCircle(mul(w, Mat4::scale(wheelRadius*0.5f, wheelRadius*0.5f, 1.0f)));
    };
...
static void initGL() {
    ...
    // IMPORTANT: disable face culling so text triangles are not discarded
    glDisable(GL_CULL_FACE);

    glDisable(GL_DEPTH_TEST);
    glViewport(0, 0, gWinW, gWinH);

    // UI projection uses framebuffer size (set in framebuffer callback too)
...
int main() {
    ...
    GLFWwindow* window = glfwCreateWindow(WIDTH, HEIGHT, "OpenGL 3.3 Core + GLFW", nullptr, nullptr);
    if (!window) {
    ...
    // init framebuffer/projection once
    int fbW, fbH;
